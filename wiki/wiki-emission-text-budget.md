...
正确性已验证（NEMU 位精确 + **性能中性**——两者缺一不可，见下方案例）的削减旋钮，才可在性能交付分支上默认开。反转有三条配套：
...
## 5. 追记（2026-08-27）：性能中性不是可选项

`GSIM_SHORT_NAMES` 曾按"正确性中性"判据翻转默认开。随后 T32 C50000 五对 A/B（同冠军种子、同调度）测出 **+4.5%**（5.338s vs 5.578s，区间不重叠；T16 同向 +1.4%）。分解实验（SHORT_NAMES=0、其余照开）证明回归全部来自 interning：**更短的标识符移动了 znver4 热函数体的代码生成对齐**。处置：SHORT_NAMES 回退默认关（按需 opt-in，注明 +4.5% 代价）；DENSE_ONLY=2 与 RESET_CHUNK 经同轮分解确认中性，保持默认开（模型 4.8GB、构建 1m18s）。

**教训**：文本整形旋钮的"中性"必须**在每个部署档位上分别测**（T16 中性 ≠ T32 中性）；判据写成"正确性中性且性能中性"，第二个词没有旁路。
